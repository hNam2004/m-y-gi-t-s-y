...
const char *mqtt_topic_info = "Kdev/TEWD43472L55/info";
const char *mqtt_topic_cmd = "Kdev/TEWD43472L55/cmd";
const char *mqtt_topic_status = "Kdev/TEWD43472L55/status";
const char *mqtt_topic_control = "Kdev/TEWD43472L55/control";
...
volatile bool forceRestartPulse = false;
...
void mqttCallback(char *topic, byte *payload, unsigned int length)
{
    ...
    else if (strcmp(topic, mqtt_topic_info) == 0)
    {
        ...
        else if (t_value && v_value && strcmp(t_value, "s") == 0 && strcmp(v_value, "1") == 0)
        {
                    int currentStatus = -1;
                    ...
                    JsonObject objs = docs.to<JsonObject>();
                    if (!digitalRead(IN_SIG1)){
                        objs["s"] = 1; 
                    }
                    else objs["s"] = 0;
                    if (infoReadSuccess) {
...
{
    long lastMsgTime = 0;
    const int publishInterval = 10000; // Gửi tin nhắn mỗi 10 giây
...
void setup()
{
    ...
    pinMode(IN_SIG1, INPUT_PULLDOWN);                                                     // Configure BOOT pin as input with internal pull-up resistor
